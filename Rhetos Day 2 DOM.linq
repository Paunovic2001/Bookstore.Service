...
void Main()
{
    ...
    using (var scope = LinqPadRhetosHost.CreateScope(rhetosHostAssemblyPath))
    {
		...
		var books = repository.Bookstore.Book.Load().Dump("Books without author (Load)");
		...
		repository.Bookstore.Book.Load().Except(books).Dump("New books");
